        ...
        <div class="md:p-8 p-5 dark:bg-purple-800 bg-white rounded">
          <div class="px-4 flex items-center justify-between">
            ...
            <div class="flex items-center">
              <button on:click={previousMonth} aria-label="calendar backward" class="text-purple-800">
                <svg
              ...
              <button on:click={nextMonth} aria-label="calendar forward" class="ml-3 text-purple-800">
                <svg
              ...
              <tbody>
                {#each rows as col}
                  <tr>
                    {#each col as i}
                      <td class="pt-4">
                        <div class="px-2 py-2 cursor-pointer flex w-full justify-center rounded hover:bg-purple-700 hover:bg-opacity-40">
                          {#if i > 0}
                            {#if inputTxt === `${i}/${selectedMonth}/${selectedYear}`}
                              ...
                              >
                            {:else}
                                <button class="border-none w-8 h-8"
                                  on:click={() => {
...
<div class={`${$$props.class} w-full flex flex-col`}>
  ...
  <div class="h-12 flex w-full items-center justify-between px-3 border border-purple-200 rounded">
    <Icon class="h-6 w-6" name="calendar" />

    <input
    type="text"
    bind:value={inputTxt}
    class="flex w-full text-center text-purple-700 focus:outline-none"
    on:click={() => {
      isOpenCalendar = true
    }}
  />
